...
while (True):
    ...
    if cashiers == 2:
        ...
        hc1 = int(
            input('Введите количество часов отработанных первым кассиром: '))  # количество часов отработанных кассиром
        ...
        print(
            'Поздравляю! Ваш штат увеличился и вам пора внести изменения в скрипт! А пока продолжайте вводить значения по старой системе.')
print('')
while (True):
    try:
...
ac = m / c  # средний чек выручка/чеки
if dc == 0:
...
if cashiers == 2:  # если кассиров в смене было двое, то данные рассчитываются на двоих
    if 0 <= c <= 50:
        if hc1 != hc2:
            c1 = (c * hc1) // (hc1 + hc2)  # (чеки * часы) / (часы1 + часы2)
            c2 = (c * hc2) // (hc1 + hc2)
            dc1 = (dc * hc1) // (hc1 + hc2)  # (дисконтные * часы) / (часы1 + часы2)
            dc2 = (dc * hc2) // (hc1 + hc2)
            print('Касса #1', cashier1)
            print('Чеки:', int(math.ceil(c1)))
            print('Проданных 7%:', dc1)
            print('')
            print('Касса #1', cashier2)
            print('Чеки:', int(math.ceil(c2)))
            ...
            print('Касса #1', cashier1)
            print('Чеки:', int(math.ceil(c1)))
            print('Проданных 7%:', int(math.ceil(dc1)))
            print('')
            print('Касса #1', cashier2)
            print('Чеки:', int(math.ceil(c2)))
            print('Проданных 7%:', int(math.ceil(dc2)))
    elif 50 < c <= 100:
    ...
    if 0 <= c <= 50:
        d1 = randint(2, 4)
        c1 = (c * hc1) // (
                hc1 + hc2 + hc3)  # формула расчёта чеков для первого кассира (остальные отталкиваются от первого, что отличается от расчёта между двумя кассирами)
        c2 = (c - c1) // 2 + d1
        c3 = c - c1 - c2
        dc1 = (dc * hc1) // (
                hc1 + hc2 + hc3)  # формула расчёта дисконтных карт для первого кассира (остальные отталкиваются от первого, что отличается от расчёта между двумя кассирами)
        dc2 = (dc - dc1) // 2
        dc3 = dc - dc1 - dc2
        print('Касса #1', cashier1)
        print('Чеки:', c1)
